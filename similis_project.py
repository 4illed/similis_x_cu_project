...
# %%
# ## Задание 2. Загрузка данных и проверка raw-части
...
# Берем первые 5 файлов из папки dataset
...
# Отключаем лимит на размер пикселей, чтобы загрузить гигантские сканы археологов
Image.MAX_IMAGE_PIXELS = None

# Оставляем только строки с реально существующими картинками (из Задания 3)
...
# 2.3 Число изображений на один артефакт (code)
...
# 2.4 Распределение базового поля (material)
...
plt.show()
# %% [markdown]
# ## Задание 4. EDA (Часть 2: Проблемные фото и визуальные режимы)
...
for path in tqdm(subset_50["full_image_path"]):
    # ИСПРАВЛЕНИЕ: Читаем через numpy, чтобы обойти баг OpenCV с кириллицей на Windows
    img_array = np.fromfile(path, dtype=np.uint8)
    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)

    # Страховка: если файл огромный и OpenCV не справился, читаем через PIL
    if img is None:
    ...
    # 3. Оцениваем тип фона (по углам картинки)
...
# --- Отрисовка проблемных и интересных режимов ---
fig, axes = plt.subplots(3, 3, figsize=(15, 12))

# Режим 1: Самые темные фото (плохое освещение)
...
# Режим 2: Самый мелкий объект в кадре (маленький foreground_ratio)
...
# Режим 3: Странный Aspect Ratio (Сильный кроп)
...
# Убедимся, что работаем с валидными данными из предыдущих шагов
valid_df = df[df["path_exists"]].copy()

print("--- 1. Вывод 15 примеров реальных описаний ---")
# Берем 15 случайных записей, где есть хоть какое-то описание
...
# Динамически считаем пропуски и количество уникальных классов для честности
candidates_data = [
    {
...
# %%
# Смотрим реальную "грязь" в данных ДО нормализации
...
# Словарь для поля MATERIAL (Материал)
# Опираемся на реальные данные: выделяем топ-классы, подтипы керамики сводим к корню.
material_dict = {
    "керамика": "керамика",
    "фаянс": "фаянс",
    "фарфор": "фарфор",
...
# Словарь для поля FRAGM (Целостность)
# Данные здесь оказались чистыми, но мы обязаны заложить страховку по ТЗ.
fragm_dict = {
    "фрагмент": "фрагмент",
    "целый": "целый",
...
# 2. Пишем функцию, которая создает 4 колонки по ТЗ
def normalize_feature(df, col_name, mapping_dict, rare_label="прочее"):
    ...
    # Очищаем текст (нижний регистр, убираем пробелы)
    cleaned_raw = df[col_name].astype(str).str.lower().str.strip()

    # Правило 1: Отмечаем пропуски
    df[missing_col] = (
        df[col_name].isna()
        | (cleaned_raw == "nan")
        | (cleaned_raw == "")
    ...
    # Правило 2: Маппинг по нашему словарю
    df[norm_col] = cleaned_raw.map(mapping_dict)

    # Правило 3: Все, что не попало в словарь (редкие классы типа "дерево", "сланец") -> в rare_label
    mask_unmapped = df[norm_col].isna() & ~df[missing_col]
    df.loc[mask_unmapped, norm_col] = rare_label

    # Правило 4: Пропуски получают токен "неизвестно"
    df.loc[df[missing_col], norm_col] = "неизвестно"

    # Правило 5: Флаг неуверенности (пропуски + редкие классы)
    df[uncertain_col] = (
...
def generate_auto_description(preds, confs, thresholds):
    ...
    # 2. Поле NAME (Обязательное, но допускает обобщение)
    ...
    # 3. Поле MATERIAL (Опциональное)
    mat_val = preds.get('material', 'неизвестно')
    mat_conf = confs.get('material', 0.0)

    # Если модель не уверена, мы просто пропускаем (не выводим) материал
...
# --- Тестирование (Строго по ТЗ: 5 обычных примеров + 1 мусорный) ---
examples = [
    # 1. Идеальный случай (высокая уверенность везде)
...
# Наш группирующий ключ - это колонка 'code' (уникальный идентификатор предмета)
GROUP_COL = 'code'
SEED = 42

print("--- 1. Создание Group-Aware Split ---")

# Оборачиваем в функцию, чтобы легко доказать воспроизводимость по SEED
def get_splits(df, group_col, random_state):
    # Шаг 1: Отделяем test_open (20%) от всей выборки
    ...
    # Шаг 2: Из оставшегося (80%) отделяем val_inner (25% от 80% = 20% от всего датасета)
...
# Рисуем распределение (в процентах!), чтобы доказать, что баланс не сломался
fig, axes = plt.subplots(1, 2, figsize=(15, 5))

def plot_dist(col, ax, title):
    # Считаем процентную долю каждого класса внутри сплита
...
# 1. Задаем итоговый размер входа
IMAGE_SIZE = 224 # Классический размер для ResNet/EfficientNet

print(f"--- 1. Настройка аугментаций (Размер входа: {IMAGE_SIZE}x{IMAGE_SIZE}) ---")

# Требование ТЗ: ResizeLongestSide + Pad
# Это спасает вытянутые артефакты от сплющивания
...
# Требование ТЗ: Безопасные аугментации для TRAIN
# Мы берем повороты и легкие изменения яркости. 
# Цветовые сдвиги (Hue) минимальны, чтобы не сломать "material"
...
# Требование ТЗ: VAL/TEST без искажений
...
# 2. Создаем класс датасета
class ArchaeologyDataset(Dataset):
    def __init__(self, df, transform=None):
        self.df = df.reset_index(drop=True)
        self.transform = transform
        
        # Словари для маппинга текста в числа (PyTorch ест только цифры)
    ...
    def __getitem__(self, idx):
        ...
        # Читаем через numpy (обход кириллицы в путях на Windows)
...
# Берем одну интересную картинку из train_df
sample_idx = 0
...
# Функция для обратной нормализации тензора в картинку
...
# 3, 4, 5. Три разных Train аугментации
for i in range(2, 5):
...
# 1. Реализуем финальный Dataset по требованиям ТЗ
class ArchaeologyDatasetFinal(Dataset):
    def __init__(self, df, transform=None):
        self.df = df.reset_index(drop=True)
        self.transform = transform
        
        # Фиксируем маппинги классов
        self.mat_classes = sorted(self.df['norm_material'].unique().tolist())
        self.fragm_classes = sorted(self.df['norm_fragm'].unique().tolist())
    ...
    def __getitem__(self, idx):
        ...
        # 2. target_mask - 1 если значение известно, 0 если 'неизвестно'
        # Это нужно, чтобы модель не штрафовали за ошибки на пустых полях
        ...
        # 3. metadata - полезная инфа для проверки
        metadata = {
...
# 3. Вывод Shapes и Dtypes для отчета
...
# %%
# Берем один элемент из валидационного лоадера
idx = 0
...
# Проверка auto_description по ground-truth (из Задания 7)
...
# Отрисовка для визуального контроля
...
# Проверка на отсутствие аугментаций в Val:
# Берем ту же картинку второй раз - она должна быть идентична первой
